    ...
    try:
        if graph == "Pressure Transducer":
            ...
            if drawgraph:
                ...
                gridgraph.clear()
                for i in range (2, len(gsmpressurelist)):
                    gridgraph.plot(np.array([float(j) for j in gsmpressurelist[0]]), np.array([int(j) for j in gsmpressurelist[i]]), gsmptcolors[i-2], label = gsmptlabels[i-2])
                for i in range (2, len(fsmpressurelist)):
                    gridgraph.plot(np.array([float(j) for j in fsmpressurelist[0]]), np.array([int(j) for j in fsmpressurelist[i]]), fsmptcolors[i-2], label = fsmptlabels[i-2])
...
def TCPClient(FSMq, GSMq):
    FSMHOST, FSMPORT = "192.168.1.109", 5557
    GSMHOST, GSMPORT = "192.168.1.109", 5555
